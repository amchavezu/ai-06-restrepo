...
\begin{frame}{The Lean workflow preserves the boundary between source and proof}
  ...
  % LEAN_STATUS_PLACEHOLDER: replace with the exact generated status and check result.
  \source{AppliedModelingLib paper-formalization workflow}
\end{frame}

\begin{frame}[fragile]{Required Lean slide: source claim, statement, and proof}
  \textbf{Original mathematical claim}
  \[
    \text{Selected equation or proposition will be inserted from the generated paper folder.}
  \]

  \textbf{Lean statement and proof fragment}
  \begin{lstlisting}
-- LEAN_CODE_PLACEHOLDER
-- Replace only with exact generated code from AR18RaceManMachine.
  \end{lstlisting}

  \textbf{What Lean verifies}

  The final version will name the exact \texttt{Spec}, identify the assumptions and conclusion, explain one nontrivial proof line, and state whether the source claim is exact or conditional.

  % LEAN_SLIDE_PLACEHOLDER: patch from the completed own run and report exact build/check result.
\end{frame}
...
\begin{frame}{Where I did not believe the AI}
  ...
  \IfFileExists{hand/wage-derivation.jpg}{%
    \centering\includegraphics[height=2.0cm]{hand/wage-derivation.jpg}%
  }{%
