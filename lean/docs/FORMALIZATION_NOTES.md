...
|---|---|---|---|---|
| Proposition 1, static equilibrium | `proposition1_static_equilibrium` | partially formalized | `ProofInterface.lean` | Exact source-shaped endpoint contains `sorry`; build did not reach the paper module. |
| Corollary 1, Cobb--Douglas output | `corollary1_cobb_douglas_output` | partially formalized | `ProofInterface.lean` | Exact source-shaped endpoint contains `sorry`; relation to Assumption 1 is recorded as ambiguous. |
| All later named results | none | not formalized | `docs/FORMALIZATION_PLAN.md` | Full source-only ledger retained; no placeholder theorem or assumed conclusion was introduced. |

## Intake Checklist

- [x] Pin the exact source bytes and version.
- [x] Complete the source-only selected-presentation inventory; semantic atom
      review remains open because the source map is not materialized.
- [x] Complete the outside-Lean formula/dependency sanity pass and working memo.
...
- [x] Create actual source definitions/models and one transparent Spec per
      selected claim in `PaperInterface.lean` for the two-row static slice.
- [x] Create each distinct proof endpoint in `ProofInterface.lean`; both remain
